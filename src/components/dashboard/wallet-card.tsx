...
import { getEtherBalance } from '@/lib/evm/alchemy';
import { cn } from '@/lib/utils';
import { setActiveWallet } from '@/server/actions/wallet';
import { revealPrivateKey } from '@/server/actions/evm-wallet-kit';
...
export function WalletCard({
  ...
  const {
    ...
    ['wallet-evm-balance', wallet.publicKey],
    () => getEtherBalance(wallet.publicKey),
    { refreshInterval: 30000 },
  );
  ...
  async function reveal() {
    ...
    const result = await revealPrivateKey(undefined);
    //console.log('result', result);
    if(result && result.data && result.data.success) {
      const privateKey = result.data.data;
